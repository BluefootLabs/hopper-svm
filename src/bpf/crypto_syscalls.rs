...
/// 32-byte output.
pub fn do_sol_blake3(ctx: &mut BpfContext, chunks: &[&[u8]], out: &mut [u8; 32]) -> SyscallResult {
    let total: usize = chunks.iter().map(|c| c.len()).sum();
    if let Err(err) = charge(ctx, hash_cost(total)) {
        return err;
    }
...
mod tests {
    ...
    fn secp256k1_recover_rejects_bad_signature() {
        ...
        let outcome = do_sol_secp256k1_recover(&mut ctx, &[0u8; 32], 0, &[0u8; 64], &mut out);
